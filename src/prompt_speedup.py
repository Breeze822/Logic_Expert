...
def translate_by_gpt(source_nl,model = "gpt-3.5-turbo"):
    file = "prompt_set/static/nl2spec_static_prompt.txt"
    final_answer = ""
...
# Our Method
def translate_by_gpt_similar(source_nl,model = "GPT35",prompt_type = "similar", temperature = 0, prompt_path ="", neigh_num = 10):
    final_answer = ""
    status = STATUS.SUCCESS
    try:
        if prompt_type == "similar":
            ...
            # file = "prompt_set/Role/mathematician_prompt.txt"
            # file = "prompt_set/Role/software_engineer_prompt.txt"
...
def translate_from_file(filename, prompt_type = 'similar',neigh = 10,engine = "gpt-3.5-turbo",prompt_path = "",output_filedir_path=""):
    ...
    for index, row in tqdm(df.iterrows(), total=df.shape[0]):
        ...
        if prompt_type == 'zero':
            ...
            nl_translation = translate_by_gpt_similar(source_nl=nl,model=engine,prompt_type = prompt_type,neigh_num=neigh,prompt_path=prompt_path)
    ...
    if output_filedir_path != "":
        ...
        # base_file_name = output_filedir_path + f"/syn_{engine}_{prompt_type}_mathematician_{rounded_value}_neigh={neigh}"
